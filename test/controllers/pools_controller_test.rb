...
class PoolsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create pool" do
    assert_difference('Pool.count') do
      post pools_url, params: { pool: { season: @pool.season, total_points: @pool.total_points, user_id: @pool.user_id, week: @pool.week } }
    end
  ...
  test "should update pool" do
    patch pool_url(@pool), params: { pool: { season: @pool.season, total_points: @pool.total_points, user_id: @pool.user_id, week: @pool.week } }
